...
| `POST /publish` | Owner-only upload of a sanitized run ZIP (maximum 4 MB) into private Blob storage and the hosted catalog. System prompt/profile files are accepted only when explicitly included in the local archive. Share credentials cannot publish. |
